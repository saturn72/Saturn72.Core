 ...
 public class WebHelper : IWebHelper
    {
        ...
        public virtual string GetSiteHost(bool useSsl)
        {
            ...
            if (!String.IsNullOrEmpty(httpHost))
            {
                ...
                var siteContext = EngineContext.Current.Resolve<IPortalContext>();
                var currentSite = siteContext.CurrentPortal;
                if (currentSite == null)
                ...
                if (useSsl)
                {
                    if (!String.IsNullOrWhiteSpace(currentSite.SecureUrl))
                    {
                ...
                {
                    if (currentSite.SslEnabled && !String.IsNullOrWhiteSpace(currentSite.SecureUrl))
                    {
        ...
        {
            if (NetCommonHelper.GetTrustLevel() > AspNetHostingPermissionLevel.Medium)
            {
                //full trust
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new Saturn72Exception("Saturn72 needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
