...
namespace Saturn72.Core.Tests.Plugins
{
    public class PluginManagerTests
    {
        ...
        {
            typeof (ArgumentNullException).ShouldBeThrownBy(() => PluginManager.MarkPluginAsInstalled(null));
        }

        [Fact]
        public void MarkPluginAsInstalled_ThrowsExceptionOnEmptySystemNameSystemName()
        {
            typeof (ArgumentNullException).ShouldBeThrownBy(() => PluginManager.MarkPluginAsInstalled(string.Empty));
        }
